...
const [proypologismoi, setProypologismoi] = useState(null);

  useEffect(() => {
const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:1337/api/proypologismoi", { headers });
        console.log(response);
        const { data } = response.data; // Extract data array from the response
        setProypologismoi(data);
        setError(null);
    ...
    if (isFired) {
      ...
      link.href = url;
      link.download =
        "{proypologismoi.attributes.PROIPOLOGISMOS2017}"; // Set the desired file name
      link.click();
